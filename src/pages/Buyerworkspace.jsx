...
export default function BuyerWorkspace() {
  ...
  useEffect(() => {
    ...
    // ===== Real-time subscriptions (new syntax) =====
  ...
  const workersForCurrentStory = storyIndex !== null
    ? liveWorkers.filter(w => w.service === formattedOrders[storyIndex]?.product_name)
    : [];
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-white p-4 pb-24">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-4 items-center">
          <FaSearch className="text-white/70 cursor-pointer" onClick={() => navigate("/shop")} />
      ...
      <div className="mb-6">
        ...
        {formattedOrders.length === 0 ? (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
            ...
            <button onClick={() => navigate("/shop")} className="bg-green-600 hover:bg-green-700 px-4 py-2 rounded-xl text-sm font-semibold transition">
              Browse Products
            </button>
          </div>
        ) : (
      ...
      <div className="mb-6 bg-white/5 border border-white/10 rounded-2xl p-4">
        ...
        {bookings.length === 0 ? (
          <div className="space-y-3">
            ...
            <button onClick={() => navigate("/hire-worker")} className="w-full bg-green-600 hover:bg-green-700 py-2 rounded-xl text-sm font-semibold transition">
              Hire a Worker
            </button>
          </div>
        ) : (
          <div className="space-y-2">
            ...
            <button onClick={() => navigate("/hire-worker")} className="w-full mt-2 bg-green-600 hover:bg-green-700 py-2 rounded-xl text-sm font-semibold transition">
              + Hire Another Worker
            </button>
          </div>
        )}
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
        <div className="flex gap-2 mb-4 items-center">
          ...
          <span className="ml-auto text-xs text-green-400">{liveWorkers.length} live</span>
        </div>
        {liveWorkers.length === 0 ? (
          ...
          <div className="space-y-2">
            {liveWorkers.slice(0, 5).map((w) => (
              <div key={w.id} className="flex justify-between items-center py-2 border-b border-white/10">
                ...
                <button onClick={() => navigate(`/hire-worker/${w.worker_id}`)} className="text-xs bg-green-600 hover:bg-green-700 px-3 py-1 rounded-full transition">
                  Hire
                </button>
              </div>
            ))}
        ...
        <button onClick={() => navigate("/live-services")} className="mt-4 w-full bg-white/10 hover:bg-white/20 py-2 rounded-xl text-sm transition">
            ...
            {workersForCurrentStory.length === 0 ? (
              ...
              workersForCurrentStory.map((w) => (
                <div key={w.id} className="flex items-center justify-between gap-4 bg-white/5 rounded-xl p-3">
                  ...
                  <button
                    onClick={() => { setStoryIndex(null); navigate(`/hire-worker/${w.worker_id}`); }}
