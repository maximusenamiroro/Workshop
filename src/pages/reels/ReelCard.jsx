...
import {
  ...
  Volume2, VolumeX, Bookmark,
} from "lucide-react";
import { useNavigate } from "react-router-dom";
import { supabase } from "../../lib/supabaseClient";
import { useAuth } from "../../context/AuthContext";

export default function ReelCard({ reel, onLikeUpdate }) {
  const { user, role } = useAuth();
  const navigate = useNavigate();
  const videoRef = useRef(null);
  const containerRef = useRef(null);

  // Likes — driven by reel.likes prop (source of truth from parent)
  // Local liked state is checked once on mount
  const [liked, setLiked] = useState(false);
  const [likeInFlight, setLikeInFlight] = useState(false); // prevent double-taps
  ...
  const toggleLike = async () => {
    ...
    setLiked(!wasLiked);
    onLikeUpdate(reel.id, wasLiked ? -1 : 1);
        ...
        <div className="flex flex-col items-center gap-4 pb-5 flex-shrink-0">
          ...
          >
            <div className="w-8 h-8 bg-black/40 backdrop-blur rounded-full flex items-center justify-center">
              <Heart
                size={19}
                className={`transition-all ${liked ? "text-red-500 fill-red-500 scale-110" : "text-white"} ${likeInFlight ? "opacity-50" : ""}`}
              />
            </div>
                ...
                comments.map((c) => (
                  <div key={c.id} className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 bg-white/5 rounded-2xl px-3 py-2">
                      <p className="text-green-400 text-xs font-semibold mb-0.5">{c.profiles?.full_name || "User"}</p>
